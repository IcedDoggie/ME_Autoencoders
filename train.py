...
def train():
	...
	for sub in range(len(total_list)):
		...
		# auto_encoder_1, encoder_1 = layer_wise_conv_autoencoder((3, 224, 224), 64, 3)		# auto_encoder_2, encoder_2 = layer_wise_conv_autoencoder(input_shape, encoded_dim, decoded_dim)
		# auto_encoder_1.compile(loss='binary_crossentropy', optimizer = adam, metrics = [metrics.categorical_accuracy])
		# encoder_1.compile(loss='binary_crossentropy', optimizer = adam, metrics = [metrics.categorical_accuracy])
		# plot_model(auto_encoder_1, show_shapes=True, to_file = 'autoencode.png')
		# plot_model(encoder_1, show_shapes=True, to_file = 'encode.png')
		...
		for X, y in loso_generator:
			...
			# Spatial Learning
			# auto_encoder_1.fit_generator(datagen.flow(X, X, batch_size = batch_size, shuffle=True), steps_per_epoch = int(len(X)/batch_size), epochs = epochs, callbacks=[history, stopping])

			conv_ae.fit_generator(datagen.flow(X, X, batch_size = batch_size, shuffle=True), steps_per_epoch = int(len(X)/batch_size), epochs = epochs, callbacks=[history, stopping])
			autoencoded_features = conv_ae.predict(X)
			...
			plot_model(vgg_model, to_file = "VGG16_Auto.png", show_shapes=True)	
		...
		# save weights
		spatial_weights_name = weights_dir + "vgg16_" + str(sub) + ".h5"
		temporal_weights_name = weights_dir + "temporal_" + str(sub) + ".h5"		
