...
def train(type_of_test, train_id, preprocessing_type, classes=5, feature_type = 'grayscale', db='Combined Dataset', spatial_size = 224, classifier_flag = 'svc', tf_backend_flag = False, attention=False, freeze_flag = 'last'):
	...
	samm2_2, _ = loading_samm_table(root_dir, sec_db, objective_flag=0)
	samm2_2 = class_merging(samm2_2)
	samm_list_2, samm_labels_2 = read_image(root_dir, sec_db, samm2_2)

	sec_db = 'SMIC_Optical_Gray_Weighted'
	smic2_2 = loading_smic_table(root_dir, sec_db)
	smic2_2 = smic2_2[0]
	smic_list_2, smic_labels_2 = read_image(root_dir, sec_db, smic2_2)	
...
f1, war, uar, tot_mat, macro_f1, weighted_f1 =  train(train_dual_stream_shallow_alexnet, 'shallow_alexnet_multi_60_MULTIPLY', preprocessing_type=None, feature_type = 'flow', db='Combined_Dataset_Apex_Flow', spatial_size = 227, classifier_flag='softmax', tf_backend_flag = False, attention = False, freeze_flag=None, classes=3)
